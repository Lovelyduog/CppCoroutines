...
struct Socket
{
    ...
    bool Bind(){
        if (bind(socket_fd_, reinterpret_cast<struct sockaddr*>(&server_addr_), sizeof(server_addr_)) == -1) {
            perror("bind");
            return false;
        }
        return true;
...
{
    AsyncIOStrategy(Socket& fd, void *buf, size_t n):fd_(fd),buf_(buf),num_(n){
...
{
    AsyncIOStrategy(Socket& fd):fd_(fd){
...
{;
    ...
    AsyncIO(Socket &fd, void *buf, size_t n):stragtegy_(fd, buf, n){
...
struct  AsyncIO<EnumAsyncIO::EnumAsyncIOClose, ReturnType>
{
    ...
    AsyncIO(Socket &fd):stragtegy_(fd){
...
AsyncIO<EnumAsyncIO::EnumAsyncIOWrite,int> AsyncWrite(Socket &fd, void *buf, size_t n){
    return  AsyncIO<EnumAsyncIO::EnumAsyncIOWrite,int>(fd, buf, n);
}

AsyncIO<EnumAsyncIO::EnumAsyncIORead,int> AsyncRead(Socket &fd,void *buf, size_t n){
    return  AsyncIO<EnumAsyncIO::EnumAsyncIORead,int>(fd, buf, n);
}

AsyncIO<EnumAsyncIO::EnumAsyncIOClose,int> AsyncClose(Socket &fd){
...
CoroutineTask<char> first_coroutine(){
    ...
    Socket file =  create_socket("./test.file", 777);
